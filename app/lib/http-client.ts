    ...
    retryPlugin({
        ...
        onRetry(config, error, count) {
            if (import.meta.env.DEV) {
                console.log(`🔄 ${config?.method?.toUpperCase()} ${config?.url} retry ${count}`);
            }
        },
...
// Cache plugin - caches successful responses
http.plugins.use(
    cachePlugin({
...
// Error cache plugin (optional) - serves cached data if request fails
// Uncomment if you want to serve stale data when requests fail
// http.plugins.use(errorCachePlugin());

/**
 * Helper function to fetch with automatic caching and retry
 * Use this instead of native fetch() in your loaders
 * 
 * Note: For binary data (like PDFs), use http.get() directly with responseType: 'arraybuffer'
 */
