...
export function Breadcrumbs() {
    ...
    const routeBreadcrumbs = matches
        ...
        .filter((crumb): crumb is { breadcrumb: React.ReactNode; pathname: string } => crumb !== null);

    // Check if we're on a blog post page
    const currentMatch = matches[matches.length - 1];
    const isBlogPost = currentMatch?.pathname?.startsWith('/blog/') && currentMatch.pathname !== '/blog';

    // Check if "Blogs" breadcrumb is already present
    const hasBlogsBreadcrumb = routeBreadcrumbs.some(crumb =>
        crumb.pathname === '/blog' || crumb.pathname === '/blogs'
    );
