...
export const meta: Route.MetaFunction = () => {
  return createMetaTags({
    title: "Setup",
    description:
      "Nischal Dahal's development setup - Tools, hardware, and software used daily. Workspace equipment, peripherals, and essential dev tools.",
    path: "/setup",
    keywords: [
      "Nischal Dahal",
      "Nischal",
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-muted-foreground">
        ...
        help me create software, and content. I also like to keep my body and
        mind healthy.
      </p>
      ...
      <div className="flex flex-col items-start gap-4">
        {keys.map((key) => (
          <div key={key} className="flex flex-col gap-2">
            <h2 className="text-sm text-primary font-stretch-ultra-expanded font-bold capitalize">
              {key}
            </h2>
            <ul className="flex gap-2 items-start rounded-md flex-wrap justify-start">
              {items[key].map((item) => (
                <li key={item.name} className="flex flex-col">
                  {item.link ? (
                    <Link
                      ...
                      target="_blank"
                    >
                      <Kbd className="text-sm! text-center! px-2 py-[0.5px]">
                        {item.name}
                      </Kbd>
                    </Link>
                  ) : (
