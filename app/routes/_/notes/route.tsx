...
const notesData: NoteItem[] = [
    {
        ...
        content: 'RSC allows rendering components on the server, reducing client-side JavaScript.',
        category: 'note',
    ...
    },
    {
        ...
        content: 'Pick, Omit, Partial, Required - essential utility types for type manipulation.',
        category: 'note',
    ...
    },
    {
        ...
        title: 'Hydration',
        term: 'Hydration',
        definition: 'The process of attaching event listeners and making server-rendered HTML interactive.',
        category: 'glossary',
        tags: ['react', 'ssr', 'web'],
        references: [
            {
    ...
    },
    {
        ...
        title: 'MDX',
        term: 'MDX',
        definition: 'Markdown for the component era. Write JSX in your Markdown documents.',
        category: 'glossary',
...
export default function Page({ loaderData }: Route.ComponentProps) {
    ...
    const filteredNotes = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            filtered = filtered.filter(note => {
                ...
                const matchesDefinition = note.definition?.toLowerCase().includes(query);
                const matchesTags = note.tags?.some(tag => tag.toLowerCase().includes(query));
                const matchesReferences = note.references?.some(ref =>
                    ref.description.toLowerCase().includes(query) ||
                    ref.title?.toLowerCase().includes(query)
                );
                return matchesTitle || matchesContent || matchesTerm || matchesDefinition || matchesTags || matchesReferences;
            });
        }
    ...
    const toggleTag = (tag: string) => {
        const newSearchParams = new URLSearchParams(searchParams);
        const currentTags = newSearchParams.get('tags')?.split(',').filter(Boolean) || [];
    ...
    const categories: { value: Category; label: string; count: number }[] = [
        ...
        { value: 'note', label: 'Notes', count: notes.filter(n => n.category === 'note').length },
        { value: 'glossary', label: 'Glossary', count: notes.filter(n => n.category === 'glossary').length },
        { value: 'bookmark', label: 'Bookmarks', count: notes.filter(n => n.category === 'bookmark').length },
    ];
                    ...
                    {note.references && note.references.length > 0 && (
                        <div className="space-y-2 mt-4">
                            ...
                            {note.references.map((ref, index) => {
                                ...
                                const refVideoId = isRefYouTube ? getYouTubeVideoId(ref.url) : null;
                                return (
